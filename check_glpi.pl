...
my $np = Nagios::Monitoring::Plugin->new(
    usage => "Usage: %s -u|--url <http://host:port/url> "
    . "[ -s|--service <service> ] "
    . "[ --ignoressl ] "
    . "[ -h|--help ] ",
    version => $version,
    ...
    extra   => "\nExample: \n"
    . "check_json.pl --url http://192.168.1.100:80/glpi -s db' ",
    url     => 'https://github.com/cconard96/check_glpi',
    plugin  => 'check_glpi',
    timeout => 15,
    shortname => "Check GLPI status API",
);

 # add valid command line options and build them into your usage/help documentation.
$np->add_arg(
    spec => 'url|u=s',
    help => '-u, --url http://user:pass@192.168.1.100:80/glpi',
    required => 1,
);
...
my $request_url = $np->opts->url."?format=json";
if ($np->opts->service) {
